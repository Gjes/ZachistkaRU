...
while {true} do {
	...
	if !(isnil "_playersel") then {
		...
		if (((_playerpos distance _basepos) > _basedist) and !(surfaceIsWater _playerpos)) then {
			...
			if (({ (_x distance _spawnpos) < ((_radarray select 0) - 50) } count (playableunits + switchableUnits)) == count (playableunits + switchableUnits)) exitwith {};
			...
			[_eGrp,_playerpos, _radarray,_markunitsarray,["AWARE", "FULL", "WEDGE"]] call fnc_ghst_waypoints;
				...
				while {true} do {
					...
					if (({ (_x distance (getposatl leader _eGrp)) > _basedist } count (playableunits + switchableUnits)) == count (playableunits + switchableUnits)) exitwith {{deletevehicle _x} foreach units _egrp; sleep 20; deleteGroup _egrp;};
		...
		if (((_playerpos distance _basepos) > _basedist) and (surfaceIsWater _playerpos)) then {
			...
			_spawnpos = [_playerpos,_radarray,true] call fnc_ghst_rand_position;
			...
			[_eGrp,_playerpos, [((_radarray select 0) * 2),((_radarray select 1)* 2)],_markunitsarray,["AWARE", "FULL", "WEDGE"],true] call fnc_ghst_waypoints;

				while {true} do {
				
					if (({ (_x distance (getposasl leader _eGrp)) > _basedist } count (playableunits + switchableUnits)) == count (playableunits + switchableUnits)) exitwith {{deletevehicle _x} foreach units _egrp; sleep 20; deleteGroup _egrp; deletevehicle _boat1;};
