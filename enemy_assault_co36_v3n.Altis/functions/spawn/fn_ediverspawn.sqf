...
[_eGrp,_patrol_mark,_radarray,_markunitsarray,["SAFE", "NORMAL", "STAG COLUMN"],true,true] call fnc_ghst_waypoints;
		...
		[_eGrp, _spawnposasl, _menlist, _sqdnum, aiSkill] spawn {
			...
			while {true} do {
				...
				if (count units _eGrp == 0) then {
					...
					_mansel = _menlist call BIS_fnc_selectRandom;
					_man = [_spawnposasl,_egrp,_mansel,_aiSkill] call fnc_ghst_create_unit;
