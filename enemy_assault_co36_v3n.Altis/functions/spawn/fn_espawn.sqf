...
_eGrp = createGroup _sideguards;
	...
	_spawnpos = [_patrol_mark,_radarray] call fnc_ghst_rand_position;
...
[_eGrp,_patrol_mark,_radarray,_markunitsarray,["SAFE", "NORMAL", "STAG COLUMN"]] call fnc_ghst_waypoints;
		...
		[_eGrp, _spawnpos, _menlist, _sqdnum, aiSkill] spawn {
			...
			while {true} do {
				...
				if (count units _eGrp == 0) then {
					...
					_mansel = _menlist call BIS_fnc_selectRandom;
					_man = [_spawnpos,_egrp,_mansel,_aiSkill] call fnc_ghst_create_unit;
